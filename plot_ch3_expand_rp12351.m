...
% Trace a ray
raypath = [1, 2, 3, 5, 1];
p0 = [0.05, 0.0, 0, 0.25, 0.7, 0] * crystal.vtx(crystal.face{raypath(1)}, :);
r0 = optics.normalize_vector([0.5, 0.2, -0.35]);
...
% cam.setOutputFmt('output/ch3_expand_rp12351_%03d.png');

canvas_fig = figure(1); clf;
set(canvas_fig, 'Color', 'w', 'Position', [0, 400, figure_size * size_factor]);

% Side view
cam.setCamPose([-120, 0.7, 15, -0.5, 0.6, 0.2]);
cam.render(fig_all);
